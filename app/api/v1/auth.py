...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    try:
        # 1) 토큰 디코드 (이 과정에서 서명 유효성/만료 여부가 1차 검증됨)
        ...
        # 2) 토큰 블랙리스트 조회
        if await TokenBlacklist.filter(jti=jti).exists():
...
async def logout(token: str = Depends(oauth2_scheme), user=Depends(get_current_user)):
    try:
        ...
        if jti and exp:
            ...
            await TokenBlacklist.create(jti=jti, expires_at=expire_dt)
        
        await logout_user(user.user_id)
        return {"message": "로그아웃 되었습니다."}
    except Exception as e:
        print(f"Logout Error Details: {e}") # 터미널에 에러 상세 출력
